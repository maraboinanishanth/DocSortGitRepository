...
{
    public partial class AutoSuggest : Form
    {
        public AutoSuggest()
        {
            InitializeComponent();
        ...
        private void AutoSuggest_Load(object sender, EventArgs e)
        {
            ...
            //pnlStart.Location = new Point(119, 420);
            pnlStart.Location = new Point(61, 420);
        ...
        internal bool isAutoSuggest = false;
        private void btnSource_Click(object sender, EventArgs e)
        {

            FileWithWordSet.Clear();
            FileWithWordSet.Capacity = 0;
            ws.wordList = new List<string>();
            ...
            if (dr == DialogResult.OK)
            {
                ...
                ConfigurationManager.AppSettings["RecentFolder"] = txtFilesFrom.Text;
                ...
                //btnAutoSuggestStart_Click(sender, e);  // Calling search member // Nishanth needs to comment this line and put autosuggestion filling words logic which trinadh put it in ReadPDF

                DirectoryInfo DirInfo = new DirectoryInfo(txtFilesFrom.Text);
                FileInfo[] FilesList1 = DirInfo.GetFiles("*.pdf");
                int filecount = Directory.GetFiles(txtFilesFrom.Text, "*.*", SearchOption.TopDirectoryOnly).Length;
                 ...
                 worker.DoWork += (se, eventArgs) =>
                {
                    ...
                    for (int i = j; i < FilesList1.Length; i++)
                    {
                        ...
                        ReadPDFAndPopulateWords(FilesList1[i]);

                        j = getProgressValue();
                        int timecount = FilesList1.Length - j;
                        // Report progress.
                        ...
                        UpdateStatus("" + FilesList1[i].Name + "", time);
                    }
                };
                 worker.RunWorkerCompleted += (se, eventArgs) =>
                 {
                     lblStatus.Text = "Your Process has been completed";
                     this.listView1.BeginInvoke((Action)(() =>
                     {
                         foreach (var wrd in ws.wordList)
                         {
                             if (!this.listView1.Items.ContainsKey(wrd) && !StopwordTool._stops.ContainsKey(wrd))
                             {
                                 this.listView1.Items.Add(wrd);
                             }
                         }
                     }));
                ...
                //foreach (FileInfo finfo in FilesList1)
                //{
                //    using (FileStream file = new FileStream(@finfo.FullName, FileMode.Open, FileAccess.Read))
                //    {
                ...
                //                }
                //                if (!System.IO.File.Exists(m_sImages + "\\" + finfo.Name.Split('.')[0] + "-" + i + ".jpg"))
                //                    bitmap.Save(m_sImages + "\\" + finfo.Name.Split('.')[0] + "-" + i + ".jpg", getImageFormat());
                //                try
                //                {
                //                    ws.fName = String.Copy(finfo.Name);
                //                    //OCR Operations ... 
                //                    MODI.Document md = new MODI.Document();
                //                    md.Create(Convert.ToString(m_sImages + "\\" + finfo.Name.Split('.')[0] + "-" + i + ".jpg"));
                ...
                //                        ws.wordList.AddRange(WordFrequency.Words(imgdata, 1, 5));
                //                        ws.wordList = ws.wordList.Distinct().ToList();
                //                        FileWithWordSet.Add(ws);
                //                        //FileWithWordSet.Distinct().ToList();
                ...
                //                        if (!System.IO.File.Exists(m_sDocFile + "\\" + finfo.Name))
                //                        {
                //                            if (System.IO.Directory.Exists(m_sImages))
                //                            {
                ...
                //                catch (Exception ex)
                //                {
                //                    MessageBox.Show(ex.Message, "Error '" + finfo.Name + "' is in use by another application");
                //                }
                //            }
                //        }
                //    }
        ...
        private void ReadPDFAndPopulateWords(FileInfo finfo)
        {
            //foreach (FileInfo finfo in FilesList1)
            //{
        ...
        //                        //Nishanth Test code start start start
        //                        if (chbUnmatchfiles.Checked == true)
        //                        {
        //                            UnMatchFileCount = UnMatchFileCount + 1;
        ...
        //                            StoreFilesInUnMatchedFolderofFileCabinet(path, FileName);//Nishanth Uncommented
        ...
        //                        }
        //                        return;
        //                        //Nishanth test code END END END
        ...
        //        //                                compareposition = 0;
        //        //                                comparesearchstringvalue = string.Empty;
        //        //                                if (CategoryValues != null)//nishanth
        //        //                                {
        //        //                                    for (int k = 0; k < CategoryValues.Length; k++)
        //        //                                    {
        //        //                                        if (CategoryValues[k].ToString() != "")
        ...
        //                            if (imgdata.Contains(SearchValues[k].ToString().ToUpper()))
        //                            {
        //                                //Nishanth Need to check why we are calling GetLeastPositionedAutoSuggestValueForAutoSuggestKeyword
        //                                AutoSuggestParentFolderValue = SearchValues[k].ToString().ToUpper();//GetLeastPositionedAutoSuggestValueForAutoSuggestKeyword(imgdata, SearchValues[k].ToString().ToUpper());
        //                                //string Values = GetLeastPositionedTextWithIndexPosition(imgdata, SearchValues[k].ToString().ToUpper(), Searchcount, compareposition, compareAutoSuggestSearchvalue, k);
        //                                if (!string.IsNullOrEmpty(AutoSuggestParentFolderValue))
        //                                {
        //                                    //string Values = GetLeastPositionedTextWithIndexPosition(imgdata, AutoSuggestParentFolderValue.ToUpper(), Searchcount, compareposition, compareAutoSuggestSearchvalue, k);
        ...
        //                                            //string AutoSuggestSubFolderValue = GetLeastPositionedAutoSuggestValueForAutoSuggestKeyword(imgdata, CategoryValues[k].ToString().ToUpper());
        //                                            //string Values = GetLeastPositionedTextWithIndexPosition(imgdata, CategoryValues[k].ToString().ToUpper(), count, compareposition, compareAutoSuggestSearchvalue, k);
        //                                            string AutoSuggestSubFolderValue = GetLeastPositionedTextWithIndexPosition(imgdata, CategoryValues[k].ToString().ToUpper(), count, compareposition, compareAutoSuggestSearchvalue, k);
        //                                            if (!string.IsNullOrEmpty(AutoSuggestSubFolderValue))
        //                                            {
        //                                                string Values = GetLeastPositionedTextWithIndexPosition(imgdata, AutoSuggestSubFolderValue.ToUpper(), count, compareposition, compareAutoSuggestSearchvalue, k);
        ...
        //                                     foreach (var wrd in ws.wordList)
        //                                    {
        //                                        if (!this.listView1.Items.ContainsKey(wrd))
        //                                        {
        //                                            //WordsList.Items.Add(wrd);
        //                                            this.listView1.Items.Add(wrd);
        //                                        }
        ...
        private void chbDefault_CheckedChanged(object sender, EventArgs e)
        {
            if (chbDefault.Checked)
            {
                ...
                pnlStart.Location = new Point(61, 463);
        ...
        private void btnAutoSuggestStart_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!Directory.Exists(txtFilesFrom.Text.Trim()))
                {
                    ...
                    pnlError.BackColor = System.Drawing.ColorTranslator.FromHtml("#f5b7b7");
                    pnlError.Location = new Point((this.Width - pnlError.ClientSize.Width) / 2, 12);
                    pnlError.Text = "Input Location doesn't exist";
                    pnlError.Location = new Point((pnlError.Width - lblError.ClientSize.Width) / 2, (pnlError.Height - lblError.ClientSize.Height) / 2);
                    status = false;
                    txtFilesFrom.Focus();
                    return;
                }
        ...
        private void listView1_ItemSelectionChanged(object sender, System.EventArgs e)
        {
            ...
            bool isParentFolder = rdCopyToParentFL.Checked;
            bool isSubFolder = rdCopyToSubFL.Checked;

            if (isParentFolder && (txtTypeOfSearchString.Text == String.Empty || txtTypeOfSearchString.Text == "Import or type one or more keywords separated by commas(,)"))
            {
                this.txtTypeOfSearchString.Text = string.Empty;
            }
            if (isSubFolder && (txtCategoryValue.Text == "Type one or more keywords separated by commas(,)"))
            {
                this.txtCategoryValue.Text = string.Empty;
            }
            ...
            if(isParentFolder)
            {
                ...
                tempWords = this.txtTypeOfSearchString.Text;//this.SelectedWordslist.Text;
            }
            if(isSubFolder)
            {
                 tempWords = this.txtCategoryValue.Text;
            }

            item = this.listView1.SelectedItems[count - 1];
            selectedWord = item.Text;

            if (isParentFolder)
            {
                if (this.txtTypeOfSearchString.Text.Contains(selectedWord))
                {
                    if (tempWords.Contains(selectedWord + ","))
                    {
                        tempWords = tempWords.Replace(selectedWord + ",", "");
                    }
                    else if (tempWords.Contains("," + selectedWord))
                    {
                        tempWords = tempWords.Replace(  "," + selectedWord , "");
                    }
                    else if (tempWords.Trim() == selectedWord.Trim())
                    {
                        tempWords = tempWords.Replace(selectedWord, "");
                    }
                ...
                {
                    if (tempWords.Length > 0) { comma =  ",";} else { comma = "";}
                    tempWords += comma + item.Text;
                    item.BackColor = Color.LightGray;
                }
                this.txtTypeOfSearchString.Text = tempWords;
            }

            if (isSubFolder)
            {
                if (this.txtCategoryValue.Text.Contains(selectedWord))
                {
                    if (tempWords.Contains(selectedWord + ","))
                    {
                        tempWords = tempWords.Replace(selectedWord + ",", "");
                ...
                this.txtCategoryValue.Text = tempWords;
            }
        }
